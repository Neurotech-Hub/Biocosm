...
export function buildSweepMarkdownReport(options: {
  ...
  if (presetDef.id === "balanced-adaptive") {
    lines.push(
      "- Note: The default BLE baseline uses firmware-aligned bursts (1 s advertise, scan window per preset) with moderate scan/advertise cadence. Asymmetric scan/advertise scheduling still drives proximity capture."
    );
  }
